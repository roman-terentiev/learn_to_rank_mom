...
def get_rets_heat(equity, save_dir):
    ...
    if sns is not None:
        ...
        im = ax.imshow(tab_display, cmap="RdYlGn", vmin=-15, vmax=15, aspect="auto")
        for i in range(tab_display.shape[0]):
            for j in range(tab_display.shape[1]):
                value = tab_display.iloc[i, j]
                if pd.notna(value):
                    ax.text(j, i, f"{value:.2f}", ha="center", va="center", fontsize=8)
        ax.set_xticks(range(tab_display.shape[1]), tab_display.columns)
        ax.set_yticks(range(tab_display.shape[0]), tab_display.index)
